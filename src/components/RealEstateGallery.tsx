			...
			<div className="max-w-7xl mx-auto">
				...
				<motion.div
					...
					<p className="text-xl text-gray-600 max-w-2xl mx-auto font-medium leading-relaxed">
						Discover tokenized real estate assets from global markets, verified
						and ready for fractional ownership.
					</p>
				</motion.div>
					...
					{[
						{
							...
							value: REAL_ESTATE_IMAGES.length,
							color: "blue",
						},
						{
						...
						},
						{
							...
							value: REAL_ESTATE_IMAGES.reduce(
						...
						<motion.div
							key={idx}
							whileHover={{ y: -5 }}
							className={`bg-white p-6 rounded-4xl border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-${stat.color}-500/5 transition-all duration-300 group`}>
							<div
								className={`text-4xl font-black text-${stat.color}-600 mb-1 group-hover:scale-105 transition-transform`}>
								{stat.value}
							</div>
						...
						{categories.map((category) => {
							...
							return (
								<motion.button
									...
									<span
										className={`w-1.5 h-1.5 rounded-full ${isActive ? "bg-white" : `bg-${category.color}-500`}`}
									/>
									{category.label}
								</motion.button>
							);
					...
					<div className="flex flex-col lg:flex-row gap-6">
						<div className="flex-1 relative">
							...
							<input
								...
								className="w-full pl-14 pr-6 py-4 bg-gray-50/50 border border-gray-100 rounded-3xl text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:bg-white transition-all font-medium"
							/>
						</div>

						<div className="flex gap-2 bg-gray-50 shrink-0 p-1.5 rounded-[1.25rem] border border-gray-100">
							<button
					...
					{filteredImages.length === 0 ? (
						<motion.div
							...
							<p className="text-gray-500 font-medium">
								Try adjusting your filters to find suitable assets.
							</p>
						</motion.div>
					) : (
						<motion.div
								...
								<motion.div
									...
									<div
										className={cn(
											...
											viewMode === "grid" ? "p-5" : "flex-1 py-2 pr-6",
										)}>
										...
										<div className="flex flex-wrap gap-2 mb-4">
											{image.tags.slice(0, 3).map((tag) => (
												<span
													key={tag}
													className="px-2.5 py-1 text-[10px] font-bold bg-gray-100 text-gray-400 rounded-lg group-hover:bg-blue-50 group-hover:text-blue-600 transition-colors uppercase tracking-tight">
													#{tag}
												</span>
											))}
										</div>
